...
class TestMcpRegistrationE2E:
    ...
    async def test_prompt_with_tool_calls_emits_acp_events(self, acp_agent, mock_manager):
        ...
        # Upstream ``ef9a08a87 fix(acp): polish Zed context and tool rendering``
        # added ``terminal`` to ``_POLISHED_TOOLS`` so ``raw_output`` is now
        # intentionally None and the content blocks carry the user-facing text.
