    ...
    **_kwargs,
    ...
    try:
        ...
        def _run_with_thread_capture():
            ...
            # Local extension on top of upstream's approval-only initializer:
            # propagate the FULL parent callback set (sudo_password, thinking,
            # etc.) into the worker thread.  Without this, only ``approval``
            # is available — sudo prompts would still fall back to input()
            # and deadlock prompt_toolkit, and reasoning callbacks would be
            # silently dropped.  See ``_run_child_with_callbacks`` above for
            # the historical context (#14443 family).
            if _apply_callbacks is not None:
                try:
                    _apply_callbacks(_parent_callbacks)
                except Exception as exc:
